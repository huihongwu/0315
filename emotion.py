...
def draw(data_path):    # csv中的聊天记录有时间顺序
    df = pd.read_csv(data_path, encoding='utf-8')
    send_df = df[df['IsSender']==1]
    receive_df = df[df['IsSender']==0]
    send_mean = send_df.groupby('StrTime').mean()
    receive_mean = receive_df.groupby('StrTime').mean()
