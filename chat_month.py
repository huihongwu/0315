...
sns_palette = sns.color_palette('PuBu', n_colors=len(month_counts))
plt.scatter(month_counts.index, month_counts.values, c=sns_palette, marker='o',s=scaled_sizes) # type: ignore
fig = plt.gcf()
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['bottom'].set_visible(True)
...
month_counts_bao.plot(kind='line', marker='o',markersize=10, linewidth=4, label='bao',color='#C6DCE4')
month_counts_hui.plot(kind='line', marker='o',markersize=10, linewidth=4, label='hui',color='#8c85be')
...
colors = ['#cedfec', '#bdd1e5', '#acc6df', '#9cb8d8', '#93a5ce', '#8c70b3', '#8a59a8']
max_day = weekday_counts.idxmax()
explode = [0.1 if day == max_day else 0 for day in weekday_counts.index]
plt.figure(figsize=(8, 8))

plt.pie(weekday_counts, explode=explode, labels=weekday_counts.index, colors=colors, autopct='%1.1f%%', shadow=True, startangle=90,textprops={'fontsize': 15}) # type: ignore
font_prop = FontProperties(family='Georgia')
...
colors = ['#e6f0f6', '#d7e5f0', '#c6d8e9', '#a4c0dc', '#98aed3', '#8f9bc9', '#8c7ab8', '#8b64ad', '#894fa3', '#831d85','#81117d', '#670864']
...
for idx, (month, count_data) in enumerate(monthly_counts.items()):
    ...
    plt.annotate(f'Max: {max_value}', xy=(max_day, max_value), xytext=(max_day-1, max_value + 1),
